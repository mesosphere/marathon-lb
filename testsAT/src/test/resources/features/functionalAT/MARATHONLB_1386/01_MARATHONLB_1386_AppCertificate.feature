...
#Deploying marathon with an app certificate
  Scenario: Deploying marathon-lb-sec with an nginx certificate
    Given I run 'cat /etc/hosts | grep nginx-qa.labs.stratio.com || echo "!{publicHostIP} nginx-qa.labs.stratio.com" | tee -a /etc/hosts' locally
    ...
    And I run 'sudo docker exec -it !{containerId} /stratio_volume/marathon-lb-app-certs.sh' in the ssh connection
